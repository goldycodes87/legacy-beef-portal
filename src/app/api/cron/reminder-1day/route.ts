...
import { build, reminderLastCall, outstandingSections } from '@/lib/email-content';
...
export async function POST(request: NextRequest) {
  ...
  for (const session of sessionsToRemind) {
    ...
    const butcherDateFormatted = new Date(animal.butcher_date).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });

    const { subject, html } = build(reminderLastCall, {
      firstName,
